        ...
        public PortfolioResponse getPortfolio(Long userId) {
                ...
                List<PortfolioResponse.InvestmentSummaryDto> investmentDtos = investments.stream()
                                .map(i -> {
                                        ...
                                        long purchasePrice = i.getPurchasePrice() > 0 ? i.getPurchasePrice() : 1; // Avoid
                                                                                                                  // divide
                                                                                                                  // by
                                                                                                                  // zero
                                        double diff = (double) (subject.getStockPrice() - purchasePrice);
                                        double pct = (diff / purchasePrice) * 100.0;
                                        String changeStr = String.format("%+.1f%%", pct);
                ...
                // 상장 폐지 내역
                List<DelistedHistory> delisted = delistedHistoryRepository
                                .findByUserIdWithTodoOrderByDelistedDateDesc(userId);
